...
from data import PLANTAS, CLIENTES, CENTROS, COSTO_PLANTA_CD, COSTO_CD_CLIENTE


def resolver() -> dict:
    prob, x, y, z, w, s = construir_modelo()
    ...
    rutas_activas = [
        (d, c) for d in D for c in C if value(w[(d, c)]) > 0.5
    ]
    ...
    # ── Desglose de costos ─────────────────────────────────────────────────────
    costo_fijos_total = sum(
        costo_fijo[d] for d in centros_abiertos
    )
    costo_transporte_pcd = sum(
        COSTO_PLANTA_CD[(p, d)] * v for (p, d), v in flujo_planta_cd.items()
    )
    costo_transporte_cdc = sum(
        COSTO_CD_CLIENTE[(d, c)] * v for (d, c), v in flujo_cd_cliente.items()
    )
    costo_penalizacion = sum(
        penal[c] * v for c, v in deficit.items()
    )
    ...
    return {
        ...
        "desglose": {
            ...
            "transporte_planta_cd": round(costo_transporte_pcd, 2),
            "transporte_cd_cliente": round(costo_transporte_cdc, 2),
